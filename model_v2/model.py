...
from model_v2.layers.downsampling import DownsamplingBlock
from model_v2.layers.upsampling import UpsamplingBlock, SubpixelShuffle
import model.audio_process as ap

class AudioUNetV2(nn.Module):
    ...
    def __init__(self, num_blocks, params, input_depth=1):
        ...
        down_num_channels = [min(max_channels, 2**(5+b))
            for b in range(1, num_blocks+1)]
        down_filter_lengths = [max(min_filter, 2**(6-b) + 1)
            for b in range(1, num_blocks+1)]
